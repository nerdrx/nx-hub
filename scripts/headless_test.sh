#!/usr/bin/env bash
# Runs NX Hub inside a headless gamescope compositor and captures a screenshot,
...
#      scripts/headless_test.sh -e NX_HUB_E2E=1 -e NX_HUB_DATA_DIR=/tmp/hub
#   Plain inherited env works too (NX_HUB_E2E=1 scripts/headless_test.sh).
#
...
LOG=$(mktemp /tmp/nx-hub-headless-XXXXXX.log)
...
echo "=== nx-hub output ==="
grep -aiE "nx-hub|\[main\]|error|Error:" "$LOG" | head -30
